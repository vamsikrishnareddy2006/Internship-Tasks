...
# ============================================================
H4 = 980
img4 = Image.new("RGB", (W, H4), BG)
d4 = ImageDraw.Draw(img4)
navbar(d4, "home")

d4.text((mx, 108), "DEVELOPER PRODUCTIVITY & CODE QUALITY DASHBOARD", font=mono(10), fill=FAINT)
d4.multiline_text((mx, 132), "Read the pulse of every commit\nbefore it becomes a problem.",
                   font=mono(28, True), fill=TEXT, spacing=10)
d4.multiline_text((mx, 210),
    "DevPulse watches the vitals behind your team's commits — volume, review\n"
    "discipline, test coverage, late-night activity — and flags risky patterns\n"
    "as Low, Medium, or High before they turn into bugs or tech debt.",
    font=sans(13), fill=DIM, spacing=6)

rr(d4, [mx, 300, mx + 190, 340], 6, fill=ACCENT)
d4.text((mx + 20, 311), "▸ run a prediction", font=mono(12, True), fill=(255, 255, 255))
rr(d4, [mx + 210, 300, mx + 400, 340], 6, outline=LINE)
d4.text((mx + 230, 311), "learn how it works", font=mono(12), fill=DIM)

# ekg strip
ey = 400
xs = list(range(mx, mx + 620))
pts = []
for x in xs:
    t = (x - mx) % 90
    y = ey
    ...
    pts.append((x, y))
d4.line(pts, fill=LOW, width=2)

cw = (W - 2 * mx - 30) / 2
c1 = [mx, 460, mx + cw, 700]
c2 = [mx + cw + 30, 460, W - mx, 700]
rr(d4, c1, 10, fill=SURFACE, outline=LINE)
rr(d4, c2, 10, fill=SURFACE, outline=LINE)
d4.text((c1[0] + 25, c1[1] + 20), "#what it reads", font=mono(12), fill=DIM)
reads = [("commits_per_week", "commit volume & cadence"), ("code_review_comments", "review engagement"),
         ("test_coverage_percent", "safety net strength"), ("late_night_commits", "fatigue & rushed changes")]
ty = c1[1] + 60
for a, b in reads:
    d4.text((c1[0] + 25, ty), a, font=mono(11), fill=TEXT)
    d4.text((c1[0] + 25, ty + 18), b, font=sans(10), fill=FAINT)
    ty += 48

d4.text((c2[0] + 25, c2[1] + 20), "#how it decides", font=mono(12), fill=DIM)
d4.multiline_text((c2[0] + 25, c2[1] + 55),
    "A Random Forest classifier reads all 10\nvitals together and returns a risk class\nwith a confidence score.",
    font=sans(11), fill=DIM, spacing=6)
by = c2[1] + 150
for lab, col in [("low", LOW), ("medium", MED), ("high", HIGH)]:
    d4.text((c2[0] + 25, by), lab, font=mono(11), fill=DIM)
    rr(d4, [c2[0] + 100, by + 3, c2[2] - 25, by + 11], 4, fill=col)
    by += 26

footer(d4, H4)
img4.save(f"{OUT}/4_home.png")

# ============================================================
# 5. About page
# ============================================================
H5 = 980
img5 = Image.new("RGB", (W, H5), BG)
d5 = ImageDraw.Draw(img5)
navbar(d5, "about")

d5.text((mx, 108), "ABOUT THIS PROJECT", font=mono(11), fill=FAINT)
d5.text((mx, 130), "Why DevPulse exists", font=mono(26, True), fill=TEXT)
d5.multiline_text((mx, 168),
    "A full-stack demonstration of connecting a trained machine learning\nmodel to a real, working web application — end to end.",
    font=sans(13), fill=DIM, spacing=6)

aimc = [mx, 240, W - mx, 400]
rr(d5, aimc, 10, fill=SURFACE, outline=LINE)
d5.text((aimc[0] + 25, aimc[1] + 20), "#aim", font=mono(12), fill=DIM)
d5.multiline_text((aimc[0] + 25, aimc[1] + 55),
    "DevPulse helps development teams monitor the health of their codebase\n"
    "and commit activity in real time — combining an ML model trained on\n"
    "developer activity metrics with a Flask backend and interactive UI to\n"
    "flag risky commit patterns before they become bugs or tech debt.",
    font=sans(12), fill=DIM, spacing=8)

c1 = [mx, 420, mx + cw, 680]
c2 = [mx + cw + 30, 420, W - mx, 680]
rr(d5, c1, 10, fill=SURFACE, outline=LINE)
rr(d5, c2, 10, fill=SURFACE, outline=LINE)
d5.text((c1[0] + 25, c1[1] + 20), "#tech stack", font=mono(12), fill=DIM)
stack = [("Frontend", "HTML, CSS, Jinja2"), ("Backend", "Flask (Python)"),
         ("Model", "scikit-learn Random Forest"), ("Storage", "SQLite"), ("Persistence", "joblib (.pkl)")]
ty = c1[1] + 60
for a, b in stack:
    d5.line([c1[0] + 25, ty + 24, c1[2] - 25, ty + 24], fill=LINE)
    d5.text((c1[0] + 25, ty), a, font=mono(11), fill=DIM)
    tw = d5.textlength(b, font=mono(11))
    d5.text((c1[2] - 25 - tw, ty), b, font=mono(11), fill=TEXT)
    ty += 40

d5.text((c2[0] + 25, c2[1] + 20), "#the model", font=mono(12), fill=DIM)
model_info = [("Target", "commit_risk"), ("Classes", "Low / Medium / High"), ("Features", "10"),
              ("Tuning", "GridSearchCV, 5-fold"), ("Test accuracy", "74.25%")]
ty = c2[1] + 60
for a, b in model_info:
    d5.line([c2[0] + 25, ty + 24, c2[2] - 25, ty + 24], fill=LINE)
    d5.text((c2[0] + 25, ty), a, font=mono(11), fill=DIM)
    tw = d5.textlength(b, font=mono(11))
    d5.text((c2[2] - 25 - tw, ty), b, font=mono(11), fill=TEXT)
    ty += 40

pipec = [mx, 700, W - mx, 800]
rr(d5, pipec, 10, fill=SURFACE, outline=LINE)
d5.text((pipec[0] + 25, pipec[1] + 18), "#pipeline", font=mono(12), fill=DIM)
flow = ["HTML form", "→", "Flask /predict", "→", "scaler.pkl", "→", "best_model.pkl", "→", "predictions.db", "→", "result page"]
fx = pipec[0] + 25
for item in flow:
    if item == "→":
        d5.text((fx, pipec[1] + 55), item, font=mono(11), fill=FAINT)
        fx += 20
    else:
        w = d5.textlength(item, font=mono(10))
        rr(d5, [fx, pipec[1] + 48, fx + w + 16, pipec[1] + 70], 5, outline=LINE)
        d5.text((fx + 8, pipec[1] + 53), item, font=mono(10), fill=DIM)
        fx += w + 28

footer(d5, H5)
img5.save(f"{OUT}/4_home.png".replace("4_home", "5_about"))
